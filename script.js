...
function updateLCD(l1, l2, l3, l4) {
    const limit = 20; // Serif font can handle about 20 chars
    let line1 = l1 || " ";
    let line2 = l2 || " ";
    let line3 = l3 || " ";
    let line4 = l4 || " ";

    // SMART WRAP: If Line 2 is too long, wrap it to Line 3
    if (line2.length > limit) {
        let lastSpace = line2.lastIndexOf(' ', limit);
        if (lastSpace === -1) lastSpace = limit;
        line3 = line2.substring(lastSpace).trim();
        line2 = line2.substring(0, lastSpace).trim();
    }
    ...
    document.getElementById('line4').innerText = line4;
}
...
let isOffHook = false, isDirectoryOpen = false, isLanguageSelected = false, currentLang = 'en', inputString = "", currentTrackNum = 1, directoryIndex = 2, volIndex = 1;
...
function toggleHandset() {
    ...
    if (isOffHook) {
        f.classList.add('up'); isLanguageSelected = false; isReviewing = false; playTrack(100);
    } else {
        if (isRecording && mediaRecorder) mediaRecorder.stop();
        f.classList.remove('up'); triggerRecoil();
        audio.pause(); audio.src = ""; isDirectoryOpen = false; inputString = ""; refreshDisplay();
    }
}
...
function startRecording() {
    ...
    setTimeout(() => {
        const beep = audioCtx.createOscillator(); beep.frequency.value = 1000; beep.connect(audioCtx.destination); beep.start(); beep.stop(audioCtx.currentTime + 0.5);
        navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
            mediaRecorder = new MediaRecorder(stream); audioChunks = []; isRecording = true;
            mediaRecorder.ondataavailable = e => audioChunks.push(e.data);
            mediaRecorder.onstop = () => { recordedBlob = new Blob(audioChunks, { type: 'audio/webm' }); isReviewing = true; refreshDisplay(); };
            mediaRecorder.start(); updateLCD("RECORDING", "SPEAK NOW", "POUND STOP", "● REC");
        });
    }, 1000);
}
...
function playTrack(num) {
    ...
    if (audioCtx && gainNode) gainNode.gain.setValueAtTime(num === 5 ? 7.0 : 1.0, audioCtx.currentTime);
    clickAudio.src = baseUrl + "0099.mp3"; clickAudio.play().catch(() => {});
    refreshDisplay();
...
function cycleVolume() { 
    ...
    const osc = audioCtx.createOscillator(), g = audioCtx.createGain();
    osc.frequency.value = 800; g.gain.setValueAtTime(0.05 * (volIndex + 1), audioCtx.currentTime);
    osc.connect(g); g.connect(audioCtx.destination); osc.start(); osc.stop(audioCtx.currentTime + 0.05);
    updateLCD("VOLUME", "LEVEL", "I".repeat(volIndex + 1), " "); setTimeout(() => { if (isOffHook) refreshDisplay(); }, 1500); 
}
function triggerRecoil() { const u = document.getElementById('main-unit'); u.classList.remove('recoil'); void u.offsetWidth; u.classList.add('recoil'); }
