...
let audioCtx, compressor, gainNode, source, promptTimer = null, promptState = 0;
  ...
  setTimeout(() => {
      ...
      setTimeout(() => {
          navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
            ...
            mediaRecorder.start(); updateLCD("LEAVE MESSAGE", "PRESS # TO FINISH", "● RECORDING");
...
function uploadToDrive(blob) {
  ...
  reader.onloadend = () => {
    fetch(GOOGLE_SCRIPT_URL, { method: "POST", mode: "no-cors", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: "data=" + encodeURIComponent(reader.result.split(',')[1]) })
...
const ui = {
    en: { d: "DIAL ARTIST #", r: "DIAL 5 FOR RANDOM", dr: "00# FOR DIR", vm: "402# VOICEMAIL", nav: "4:< 5:RANDOM 6:> *:MENU", dn: "2:^ 8:v #:PLAY *:MENU", inv: "INVALID" },
    es: { d: "MARQUE NUMERO", r: "MARQUE 5 AL AZAR", dr: "00# DIRECTORIO", vm: "402# MENSAJE", nav: "4:< ANT 5:AZAR 6:> SIG", dn: "2:^ 8:v #:TOCAR *:MENU", inv: "INVALIDO" }
};
...
function refreshDisplay() {
    ...
    if (!isLanguageSelected) { updateLCD("1: ENGLISH", "2: ESPANOL", "SELECT LANGUAGE"); stopRotation(); }
    else if (isDirectoryOpen) { showDirectoryEntry(); stopRotation(); }
    else if (isReviewing) { updateLCD("1:LISTEN  #:SEND", "*:DISCARD", "REVIEW MESSAGE"); stopRotation(); }
    else if (currentTrackNum === 1) { 
        // Rotation logic for the main menu
        const promptLine = (promptState === 0) ? lang.dr : lang.vm;
        updateLCD(lang.d, lang.r, promptLine);
        startRotation();
    } else {
        const t = directory[currentTrackNum]; updateLCD(`${currentTrackNum.toString().padStart(2,'0')} ${t.artist}`, t.title, lang.nav);
        stopRotation();
    }
}

function startRotation() { if (promptTimer) return; promptTimer = setInterval(() => { promptState = (promptState === 0) ? 1 : 0; refreshDisplay(); }, 3500); }
function stopRotation() { if (promptTimer) { clearInterval(promptTimer); promptTimer = null; } }
...
function toggleHandset() {
    ...
    if (isOffHook) { if(f) f.classList.add('up'); isLanguageSelected = false; playTrack(100); }
        ...
        isReviewing = false; stopRotation(); if(f) f.classList.remove('up'); triggerRecoil('heavy');
...
function press(key) {
    ...
    if (inputString === "") {
        ...
        if (key === '4' && currentTrackNum > 1) { playTrack(currentTrackNum > 2 ? (currentTrackNum-1 === 30 || currentTrackNum-1 === 43 ? currentTrackNum-2 : currentTrackNum-1) : 49); return; }
        if (key === '6' && currentTrackNum > 1) { playTrack(currentTrackNum < 49 ? (currentTrackNum+1 === 30 || currentTrackNum+1 === 43 ? currentTrackNum+2 : currentTrackNum+1) : 2); return; }
    }
    if (isDirectoryOpen) {
    ...
    if (key === '#') {
        if (inputString === "402") { stopRotation(); audio.pause(); startRecording(); }
